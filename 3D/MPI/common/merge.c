...
/* > ./merge.out FILE_DIRECTORY OUTPUT_DIRECTORY */
{
  ...
  /* Check Z-array data */
  cnt=0;
  sprintf(filname,"%sz_%05d.dat",fildir,cnt++);
  if ((infil=fopen(filname,"r"))!=NULL){

    while(fscanf(infil,"%lf",&dummy) !=EOF) nz++;
    mpiz++;
    nzall+=(nz-2*zoff);
    fclose(infil);
    ...
    printf("Z grid number per 1 MPI process is %d.\n",nz);
    printf("MPI process number along Z is %d.\n",mpiz);
  ...
  for (m=0;m<mpiz;m++){
    ...
    fclose(infil);
    for (k=0;k<nz-2*zoff;k++){
      z[m*(nz-2*zoff)+k]=ztmp[k+zoff];
    }
  }
  ...
  int mpi=mpix*mpiy*mpiz,mxy=mpix*mpiy,flag;
  float *val[smax],*merge_val[smax];
  for (s=0;s<smax;s++){
    val[s]=(float*)malloc(sizeof(float)*nx*ny*nz);
  ...
  for (n=nstt;n<nt;n++){
    ...
    for (m=0;m<mpi;m++){
  	...
  	/* Read */
  	for (s=0;s<smax;s++){
  	  itmp=fread(val[s],sizeof(float),nx*ny*nz,infil);
	  for (k=0;k<nz-2*zoff;k++){
	    for (j=0;j<ny-2*yoff;j++){
	      for (i=0;i<nx-2*xoff;i++){
		merge_val[s][nxall*(nyall*(mz*nzall/mpiz+k)+(my*nyall/mpiy+j))+(mx*nxall/mpix+i)]=val[s][nx*(ny*(k+zoff)+(j+yoff))+(i+xoff)];
	      }
  	    }
  	  }
  	}
